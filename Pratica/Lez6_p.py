...
def reverse_recursive(s: str) -> str:
    ...
    if len(s) == 2:
        return s[1]+s[0]
    return s[-1] + reverse_recursive(s[1:-1]) + s[0]
...
def knapsack(l: List[Tuple[int, int]], W: int) -> List[Tuple[int, int]]:
    """
    [(10, 60), (20, 100), (30, 120)]
    (10, 60) -> cerco tra il resto quelli che hanno peso < W-10
    """
    def _knapsack(l, W):
        if len(l) <= 1:
            return l
        
        current_weight, current_sum = 0, 0
        binary_lenght = len(l)-1
        result = []

        for i in range(0, 2**binary_lenght):
            current_binary = bin(i >> int("0"*binary_lenght, base=2))[2:]
            current_binary = "0"*(binary_lenght-len(current_binary)) + current_binary
            current_weight, current_sum = l[0][0], l[0][1]
            for j in range(len(current_binary)):
                if current_binary[j] == "1":
                    current_weight += l[j+1][0]
                    current_sum += l[j+1][1]
                if current_weight > W:
                    break
                    
                if j == len(current_binary)-1:
                    result += [(current_weight, current_sum)]
                    
        l.pop(0)
        result += (_knapsack(l, W))
        
        return result
    
    result = max([x[1] for x in _knapsack(l, W)])
    return result
