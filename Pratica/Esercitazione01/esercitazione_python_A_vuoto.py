...
def num_digits(intero):
    if intero == 0:
        return 1
    return int(math.floor(math.log10(intero)))+1
...
def check_int_str(stringa, intero):
    ...
    #     if stringa[i:i+len(str(intero))] == str(intero):
    #         counter += 1
...
def list_multipli(L):
    result = list(filter(lambda x: x%2==0, L)) + list(filter(lambda x: x%5==0, L))
    return result
...
def get_list_except_min_max_general(L):
    if not len(L):
        return 0
    max_num, min_num = max(L), min(L)
    number_of_max, number_of_min = L.count(max_num), L.count(min_num)
    i = 0
    while i < max(number_of_max, number_of_min):
        if max_num in L: 
            L.remove(max_num)
        if min_num in L:
            L.remove(min_num)          
...
def sort_by_str(T):
    ...
    result = sorted(T, key=len)
    for i in range(len(result)):
        for j in range(len(result)):
            if i != j:
                if len(result[i]) == len(result[j]):
                    for k in range(len(result[i])):
                        if ord(result[i][k]) < ord(result[j][k]):
                            break
                        elif ord(result[i][k]) > ord(result[j][k]):
...
def sort_by_month(L):
    ...
    result = sorted(L, key=lambda x: mesi.index(x.lower()))
    return result
...
# print(anagramma("A gentleman", "amor"))
