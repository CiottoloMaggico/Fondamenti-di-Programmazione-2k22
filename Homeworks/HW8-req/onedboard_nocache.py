...
def tree_builder(table, player, opponent, params, empties, score):
    a, b, c = 0, 0, 0
    if not empties:
        disks = {"B": table.count("B"), "W": table.count("W")}
        return score[max(disks, key=disks.get) if not disks["B"] == disks["W"] else "ODD"]
    ...
    for index in empties.copy():
        ...
        if board is not False:
            ...
            points = tree_builder(
                board, opponent, player, params, empties, score)
            empties += index,
...
def dumbothello(filename: str) -> tuple[int, int, int]:
    with open(filename, "r") as board:
        board = list(
            map(str, "".join(["".join(line.split())+" " for line in board.readlines()])))
